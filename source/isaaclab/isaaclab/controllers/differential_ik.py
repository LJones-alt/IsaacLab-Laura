...
# class DifferentialIKController_MOD:
...
#         #print(f"[CONTROLLER IK] Joint pos {joint_pos.squeeze()}")
#         #print(f"[CONTROLLER IK] Delta joint pos {delta_joint_pos.squeeze()}")
#         #print(f"[CONTROLLER IK] Desired joint pos {joint_pos + delta_joint_pos}")
...
#        # print(f"[CONTROLLER IK] Delta joint pos {delta_joint_pos}")
...
class DifferentialIKController:
    ...
    def compute(
        self, ee_pos: torch.Tensor, ee_quat: torch.Tensor, jacobian: torch.Tensor, joint_pos: torch.Tensor, grads: torch.tensor=torch.tensor([[1,1,1,1,1,1,1]], device="cuda:0")
    ) -> torch.Tensor:
        """Computes the target joint positions that will yield the desired end effector pose.
        ...
        ## for debug
        # print(f"[DEBUG IK] controller recieved: \n") 
        ...
        if "position" in self.cfg.command_type:
           ...
           # print(f"[DEBUG IK] got position error {position_error} axis {axis_angle_error}")
            pose_error = torch.cat((position_error, axis_angle_error), dim=1)
            #print(f"[DEBUG IK] got pose error {pose_error}")
            delta_joint_pos = self._compute_delta_joint_pos(delta_pose=pose_error, jacobian=jacobian, grads=grads)
        # return the desired joint positions
        #print(f"[CONTROLLER IK] Joint pos {joint_pos.squeeze()}")
        #print(f"[CONTROLLER IK] Delta joint pos {delta_joint_pos.squeeze()}")
        #print(f"[CONTROLLER IK] Desired joint pos {joint_pos + delta_joint_pos}")
    ...
    def _compute_delta_joint_pos(self, delta_pose: torch.Tensor, jacobian: torch.Tensor, grads: torch.tensor= torch.tensor([[1,1,1,1,1,1,1]], device="cuda:0")) -> torch.Tensor:
        """Computes the change in joint position that yields the desired change in pose.
        ...
        if self.cfg.ik_method == "pinv":  # Jacobian pseudo-inverse
            ...
            # parameters
            lambda_val = self.cfg.ik_params["lambda_val"]
            #print(f"[DEBUG IK] lambda val {lambda_val}")
            # computation
            jacobian_T = torch.transpose(jacobian, dim0=1, dim1=2)
            #print(f"[DEBUG IK] jacobian t : {jacobian_T}")
            lambda_matrix = (lambda_val**2) * torch.eye(n=jacobian.shape[1], device=self._device)
            #print(f"[DEBUG IK] lambda matrix {lambda_matrix}")
            delta_joint_pos = (
                jacobian_T @ torch.inverse(jacobian @ jacobian_T + lambda_matrix) @ delta_pose.unsqueeze(-1)
            )
            #print(f"[DEBUG IK] delta_joint_pos {delta_joint_pos}")
            delta_joint_pos = delta_joint_pos.squeeze(-1)
        else:
            raise ValueError(f"Unsupported inverse-kinematics method: {self.cfg.ik_method}")
        #print(f"[CONTROLLER IK] Delta joint pos {delta_joint_pos}")
        modified_deltas = delta_joint_pos * grads
        #print(f"[CONTROLLER IK] Modified deltas : {modified_deltas} ")
        return delta_joint_pos
