...
from isaaclab.controllers.differential_ik import DifferentialIKController#, DifferentialIKController_MOD
...
# class DifferentialInverseKinematicsAction_MOD(ActionTerm):
...
#         self._ik_controller = DifferentialIKController_MOD(
...
#         print(f"[CONTROLLER] recieved actions : {self._raw_actions.squeeze()}")
...
#        # print(f"[CONTROLLER] Joint pos target : {joint_pos_des.squeeze()}")
#       #  print(f"[CONTROLLER] joint pos : {joint_pos.squeeze()}")
...
class DifferentialInverseKinematicsAction(ActionTerm):
    ...
    def apply_actions(self, grads: torch.Tensor=torch.tensor([[1,1,1,1,1,1,1]], device="cuda:0")):
        # obtain quantities from simulation
       # print(f"[task space] called")
        ee_pos_curr, ee_quat_curr = self._compute_frame_pose()
        joint_pos = self._asset.data.joint_pos[:, self._joint_ids]
        # compute the delta in joint-space
        if ee_quat_curr.norm() != 0:
            jacobian = self._compute_frame_jacobian()
            joint_pos_des = self._ik_controller.compute(ee_pos_curr, ee_quat_curr, jacobian, joint_pos, grads)
        else:
            joint_pos_des = joint_pos.clone()
        # set the joint position command
        self._asset.set_joint_position_target(joint_pos_des, self._joint_ids)
