...
from isaaclab.controllers import FrankaJointControllerActionCfg, FrankaJointControllerAction, FrankaGripperBinaryActionCfg
##
# Pre-defined configs
##
...
class EventCfg():
    ...
    reset_object_position = EventTerm(
        ...
        mode="reset",
        params={
            "pose_range": {"x": (0,0.1), "y": (-0.1,0.1), "z": (0.022, 0.022)},
            "velocity_range": {},
            "asset_cfg": SceneEntityCfg("object", body_names="object"),
...
class FrankaDevEnvVMCfg(dev_env_cfg.FrankaDevEnvCfg):
    ...
    #   # apply_semantic_label(self.scene.robot.prim_path, "Robot")
       # apply_semantic_label(self.scene.table.prim_path, "table")
        ...
        # Joint Position action term: receives 7 target joint positions (one per panda_joint)
        # and drives them via the articulation's implicit PD actuators.
        self.actions.arm_action =mdp.JointPositionActionCfg(
            asset_name="robot",
            joint_names=["panda_joint.*"],
            control_mode="position", 
            #offset= [0.1628, 0.0, 0.0] # Set to "position", "velocity", or "torque"
            # p_gains=[600.0, 600.0, 600.0, 600.0, 250.0, 150.0, 50.0],
            # d_gains=[50.0, 50.0, 50.0, 50.0, 30.0, 20.0, 10.0],
        )

        self.actions.grippper_action= FrankaGripperBinaryActionCfg(
            asset_name="robot",
...
def configure_robot_actuators_for_torque_mode(robot_cfg: ArticulationCfg):
    """CRITICAL: If using control_mode='torque', zero out implicit PD gains in PhysX
    so the simulator does not apply competing drive forces.
    """
    for actuator_name, actuator_cfg in robot_cfg.actuators.items():
        if "panda" in actuator_name:
