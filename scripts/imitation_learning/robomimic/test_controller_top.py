...
class TestController:
    def __init__(self, device, env, gripper_orient:int=0):
        ...
        self.rest_rot = torch.tensor([[ 0.0,  0.0, 0, 0]], device=self.device)
        ...
        self.offset_pos = torch.tensor([[0.0, 0.0, 0.05]], device=self.device)
        self.offset_quat = torch.tensor([[0.0, 0.0, 0, 0]], device=self.device)
        self.robot_entity_cfg , self.ee_jacobi_idx = self._setup_robot()
        self.gripper_orient=0
        self._init_task()
    ...
    def _get_goal_pose(self):
        ...
        match self.state:
            ...
            case TaskState.APPROACH_GOAL:
                self.gripper_state=GripperState.CLOSE
                position= torch.add(self.object_goal_pos, torch.tensor([0, 0, 0.13], device=self.device))
                return position, self.current_ee_rot #self.rest_rot
    ...
    def get_action(self):
        ...
        #print(f"[DEBUG] Position error {torch.norm(self.current_ee_pos - desired_pos)}")
        #print(f"[DEBUG] Rotation error : {torch.norm(self.current_ee_rot - desired_rot)}")
        #print(f"[DEBUG] current Rot : {self.current_ee_rot} Desired rot { desired_rot}")
        match self.state:
            case TaskState.REST:
